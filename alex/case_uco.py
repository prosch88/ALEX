...
def parse_size(size_str: str) -> int:
    ...
    if not match:
        raise ValueError(f"Not a valid value: {size_str}")
...
def backup_case_json(case_device, case_backup):
    ...
    device_uuid = uuid.uuid5(uuid.NAMESPACE_DNS, "alex" + case_device["serialNumber"])
    backup_case = []
    tool = f"ALEX {case_backup['tool_version']}"
    ...
    tool_entry = {
            ...
            "uco-core:name": "Android Logical Extractor (ALEX)",
    ...
    org_name = (case_device["serialNumber"]).upper()
    org_uuid = uuid.uuid5(uuid.NAMESPACE_DNS, org_name)

    organization = {
        "@id": "kb:organization-" + str(org_uuid),
    ...
    storage_out = 0
    try: 
        storage_out = parse_size(storage_in)
        md_facet["uco-observable:storageCapacityInBytes"] = storage_out
    except:
        pass
    ...
    software_type = os_dict.get(d_type, "Android")
